...
DMG_PATH="dist/KortexOats(Dev).dmg"
...
TEMP_DMG="dist/KortexOats(Dev)_temp.dmg"
...
hdiutil create -volname "KortexOats(Dev)" -srcfolder "$STAGING_DIR" -ov -format UDRW "$TEMP_DMG"
...
tell application "Finder"
  tell disk "KortexOats(Dev)"
    ...
    set position of item "KortexOats(Dev).app" of container window to {120, 150}
