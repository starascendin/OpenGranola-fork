...
final class AppCoordinator {
    ...
    private func handleMeetingDetected(app: MeetingApp?) async {
        ...
        // Preserve the old fork behavior: start automatically when a meeting is detected.
        // We still honor previously dismissed apps to avoid repeatedly starting on false positives.
    ...
    private func handleDetectionAccepted() {
        ...
        Task {
            ...
            self.startDetectedSession(app: app)
        }
    }

    private func startDetectedSession(app: MeetingApp?) {
        ...
        handle(.userStarted(metadata), settings: activeSettings)
    }
